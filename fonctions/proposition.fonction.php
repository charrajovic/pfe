...
function verfie_email_exist($email)
{
  $res=$GLOBALS['conn']->query(
  	"SELECT email FROM `utilisator` WHERE email='$email' AND email!='{$_SESSION['email']}'"
...
{
	$res=$GLOBALS['conn']->query("SELECT nom FROM `utilisator` WHERE email='$email'");
...
function envoyer_fichier($nom_fichier,$location_fichier,$message,$objet,$nom_des,$exp,$email_des,$email_expe)
{
 ...
 move_uploaded_file($location_fichier,$_SERVER['DOCUMENT_ROOT'].'\mini_reseau\upload\\'.$nom_fichier);
       $GLOBALS['conn']->query("INSERT INTO `conversation`(`id_message`,  `sujet`, `message`, `file`, `datep`) VALUES ('','$objet','$message','$nom_fichier',NOW())") or die(mysql_error());
