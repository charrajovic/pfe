...
if (isset($_POST['first_name']) AND !empty($_POST['first_name'])) {
  $nom = $_POST['first_name'];
   echo $nom;
  $conn->query("UPDATE user SET  nom = '$nom' WHERE id ='$id'" );
  $_SESSION['nom']=$nom;

} else if (isset($_POST['last_name']) AND !empty($_POST['last_name'])) {
  $prenom = $_POST['last_name'];
  echo $prenom;
  $conn->query("UPDATE `user` SET prenom ='$prenom' WHERE id ='$id'"  );
...
} else if (isset($_POST['job']) AND !empty($_POST['job']))
{
  $travail = $_POST['job'];
   ...
   echo $travail;
  $conn->query("UPDATE `user` SET travail = '$travail'  WHERE id = '$id'" );
